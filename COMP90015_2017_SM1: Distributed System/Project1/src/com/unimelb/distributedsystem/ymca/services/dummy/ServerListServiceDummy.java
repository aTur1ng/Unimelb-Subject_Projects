...
public class ServerListServiceDummy implements ServerListService
{
	...
	private static Set<String>	filterOut	= new HashSet<String>(Arrays.asList(new String[]
	{ "localhost", "127.0.0.1", "0.0.0.0" }));

	@Override
	public synchronized List<ServerAddress> getServerList(ServerAddress myServer)
	{
		List<ServerAddress> retVal = set.stream().filter(e -> !filterOut.contains(e)).collect(Collectors.toList());
	...
	public synchronized void addServers(List<ServerAddress> addresses) throws EzShareException
	{
		...
		{
			if (!filterOut.contains(address.getHostname()))
			{
				LoggerUtil.println("Adding to server list: " + address);
				set.add(address);
			}
			else
			{
				LoggerUtil.println("Invalid server address: " + address);
			}
		}
